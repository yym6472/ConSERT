...
from data_utils import load_datasets, save_samples, load_senteval_binary, load_senteval_sst, load_senteval_trec, load_senteval_mrpc
...
def parse_args():
    ...
    parser.add_argument("--train_data", type=str, choices=["sst2","trec","mrpc","mr","cr","subj","mpqa","nli", "stssick"], default="nli", help="Training data, on NLI or STS dataset")
    ...
    parser.add_argument("--save_samples_only", action="store_true", help="Only save training samples and do not train model")
    ...
    parser.add_argument("--data_augmentation_strategy", type=str, default="adv", choices=["adv", "none", "meanmax", "shuffle", "cutoff", "shuffle-cutoff", "shuffle+cutoff", "shuffle_embeddings", "span"], help="The data augmentation strategy in contrastive learning")
...
#     time_str = datetime.now().strftime("%Y%m%d%H%M%S")
    ...
    if args.save_samples_only:
        save_samples(train_samples, os.path.join(model_save_path, "train_data.txt"))
        sys.exit(0)
